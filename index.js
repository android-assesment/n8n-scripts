...
(function() {
    // Pehle check karein ki custom element pehle se define to nahi hai
    if (window.customElements.get('n8n-chat-embed')) {
        return;
    }

    class N8NChatEmbed extends HTMLElement {
        constructor() {
            super();
            // Shadow DOM ko attach karein taaki styling isolate rahe
            this.attachShadow({ mode: 'open' });
        }

        // Yeh function tab chalta hai jab element ko page mein joda jaata hai
        connectedCallback() {
            this.config = this.getConfig();
            if (!this.config.hostname) {
                console.error("N8N Chat Error: 'hostname' attribute <n8n-chat-embed> tag mein zaroori hai.");
                return;
            }
        ...
        // Custom element ke attributes se configuration ko padhein
        getConfig() {
            return {
                hostname: this.getAttribute('hostname'),
                label: this.getAttribute('label') || 'Chat Karein',
                placeholder: this.getAttribute('placeholder') || 'Apna sandesh likhein...',
                primaryColor: this.getAttribute('primary-color') || '#3b82f6',
                textColor: this.getAttribute('text-color') || '#ffffff',
                botMessageColor: this.getAttribute('bot-message-color') || '#f1f5f9',
                botMessageTextColor: this.getAttribute('bot-message-text-color') || '#1e293b',
                initialMessage: this.getAttribute('initial-message') || 'Salaam! Main aapki kaise madad kar sakta hoon?',
            };
        }

        // Chat ka poora HTML aur CSS banayein
        render() {
            const style = `
                :host {
                ...
                @media (max-width: 400px) {
                    .n8n-chat-window {
                        width: 100vw; height: 100%; max-height: 100%;
            ...
            const html = `
                <div class="n8n-chat-container">
                    <button class="n8n-chat-bubble">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>
                    </button>
                    <div class="n8n-chat-window">
        ...
        addEventListeners() {
            this.elements.bubble = this.shadowRoot.querySelector('.n8n-chat-bubble');
            this.elements.closeButton = this.shadowRoot.querySelector('.n8n-close-button');
            this.elements.form = this.shadowRoot.querySelector('.n8n-chat-form');
            this.elements.input = this.shadowRoot.querySelector('input');
            this.elements.window = this.shadowRoot.querySelector('.n8n-chat-window');
            this.elements.messages = this.shadowRoot.querySelector('.n8n-chat-messages');
    ...
    // Naye custom element ko browser mein define/register karein
    window.customElements.define('n8n-chat-embed', N8NChatEmbed);
})();
